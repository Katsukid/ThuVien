...
<xsl:stylesheet version="1.0"
 ...
 <xsl:when test="$leader6='a'">
 <xsl:choose>
 <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'">Văn bản</xsl:when>
 ...
 <xsl:when test="$leader6='t'">Văn bản</xsl:when>
 ...
 <xsl:when test="$leader6='k'">Ảnh</xsl:when>
 <xsl:when test="$leader6='r'">Đối tượng</xsl:when>
 ...
 <span class="label">Tài liệu chủ: </span>
 <a>
 <xsl:choose>
...
<!-- Other classification -->
 ...
 <span class="results_summary opac_view"><span class="label">Xem hiển thị OPAC: </span>
 <a><xsl:attribute name="href"><xsl:value-of select="$OPACBaseURL"/>/cgi-bin/koha/opac-detail.pl?biblionumber=<xsl:value-of select="str:encode-uri(marc:datafield[@tag=999]/marc:subfield[@code='c'], true())"/></xsl:attribute><xsl:attribute name="target">_blank</xsl:attribute>Mở trong cửa sổ mới</a>.
 </span>
 </xsl:if>
 </xsl:otherwise>
 </xsl:choose>
 ...
 <!-- #13383 -->
 <xsl:when test="@tag&lt;700"><span class="byAuthor">Liệt kê theo: </span></xsl:when>
